...
task TratarSensores(){
	while(true){
		if(ValorSensor[Acelerometro] < -190){
			StatusAcel = Inclinado;
		}
		else
			StatusAcel = NaoInclinado;
		if(ValorSensor[SonarEsquerdo] < ValorSensor[SonarDireito]){
			UltimoLado = Esquerda;
		}
		else if(ValorSensor[SonarDireito] < ValorSensor[SonarEsquerdo]){
			UltimoLado = Direita;
		}
		if(ValorSensor[Acelerometro] != 0){
			StatusSonar = Frente;
		}
		else{
